  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://127.0.0.1:8000/api/v1/bayar/buktiBayar/${idPemesanan}`,
        formData,
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        setBayarPesanan((prevState) =>
          prevState.filter(
            (historyItem) =>
              !historyItem.detail_pemesanan.some(
                (detailItem) => detailItem.id_pemesanan === idPemesanan
              )
          )
        );
        closeModal();
              ...
              {bayarPesanan.map((historyItem, historyIndex) =>
                historyItem.detail_pemesanan.map((detailItem, detailIndex) => (
                  <tr key={`${historyIndex}-${detailIndex}`}>
                    ...
                    <td className="text-center">
                      {detailItem.hampers
                        ? detailItem.hampers.nama_hampers
                        : "-"}
                    </td>
                    <td className="text-center">
      ...
      {isModalOpen && (
        <dialog id="my_modal_1" className="modal" open>
          <div className="modal-box">
            ...
            <hr />
            <h3 className="font-bold text-xl text-white mb-3 mt-2">
              Transfer Bank Only
            </h3>
