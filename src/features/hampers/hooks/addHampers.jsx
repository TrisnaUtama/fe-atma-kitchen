...
export default function useAddHampers() {
  const [validation, setValidation] = useState();
  const navigate = useNavigate();
  const handlerAddHampers = useCallback(async (formData) => {
    ...
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/api/v1/hampers/add`,
        formDataObj
      );
      navigate("/hampers");
  ...
  return { handlerAddHampers, validation };
}
